...
`DISTRIBUTED` 关键字用于给表 [创建分桶](/sql-reference/sql-statements/data-definition/CREATE%20TABLE.md#distribution_desc)，以上示例中使用 `recruit_date` 以及 `region_num` 两个字段作为分桶列。并且自 2.5.6 起 StarRocks 支持自动设置分桶数量，您无需手动设置分桶数量，详见 [确定分桶数量](../table_design/Data_distribution.md#确定分桶数量)。
